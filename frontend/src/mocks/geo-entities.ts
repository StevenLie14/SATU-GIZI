import { type GeoEntity } from "@/types";
import vendorImg from "@/assets/vendor.jpg";
import schoolImg from "@/assets/school.jpg";
import kitchenImg from "@/assets/kitchen.jpg";

export const mockEntities: GeoEntity[] = [
  // Vendor Data
  {
    id: "v1",
    name: "PT Maju Tani Pangan",
    type: "vendor",
    lat: -6.21,
    lng: 106.83,
    address: "Jl. Tebet Timur Dalam, Jakarta Selatan",
    status: "active",
    rating: 4.8,
    commodities: ["Beras", "Sayuran Segar", "Bumbu Dapur"],
    description: "Pemasok bahan pangan pokok yang berfokus pada kualitas gabah dan sayuran organik langsung dari petani binaan di Jawa Barat.",
    image: vendorImg,
    phone: "021-1234567",
  },
  {
    id: "v2",
    name: "CV Berkah Lauk",
    type: "vendor",
    lat: -6.19,
    lng: 106.84,
    address: "Pasar Senen Blok III, Jakarta Pusat",
    status: "active",
    rating: 4.6,
    commodities: ["Daging Sapi", "Daging Ayam", "Telur"],
    description: "Spesialis penyedia protein hewani segar yang telah bersertifikasi Halal dan melalui kontrol kualitas ketat dari dinas peternakan.",
    image: vendorImg,
    phone: "021-7654321",
  },
  {
    id: "v3",
    name: "Koperasi Tani Nusantara",
    type: "vendor",
    lat: -6.92,
    lng: 107.6,
    address: "Jl. Pasir Kaliki, Bandung",
    status: "active",
    rating: 4.9,
    commodities: ["Buah-buahan", "Sayuran Organik", "Susu Sapi"],
    description: "Koperasi petani lokal yang menyediakan produk segar pegunungan dengan sistem bagi hasil yang adil bagi komunitas petani Bandung.",
    image: vendorImg,
    phone: "022-9998887",
  },
  {
    id: "v4",
    name: "Agen Sayur Segar Ciawi",
    type: "vendor",
    lat: -6.65,
    lng: 106.85,
    address: "Pasar Ciawi, Bogor",
    status: "active",
    rating: 4.7,
    commodities: ["Sayuran Segar", "Kentang", "Tomat"],
    description: "Pusat distribusi sayuran segar dari kawasan Puncak Bogor, menjamin kualitas 'farm-to-table' dengan pengiriman harian.",
    image: vendorImg,
    phone: "0251-1112223",
  },
  {
    id: "v5",
    name: "Toko Sembako Makmur",
    type: "vendor",
    lat: -6.22,
    lng: 106.81,
    address: "Kebayoran Baru, Jakarta Selatan",
    status: "active",
    rating: 4.5,
    commodities: ["Beras", "Minyak Goreng", "Gula"],
    description: "Grosir sembako terpercaya di Jakarta Selatan yang telah melayani kebutuhan pangan pokok selama lebih dari 10 tahun.",
    image: vendorImg,
    phone: "021-3334445",
  },
  {
    id: "v6",
    name: "Distributor Daging Sapi Halal",
    type: "vendor",
    lat: -6.15,
    lng: 106.9,
    address: "Kelapa Gading, Jakarta Utara",
    status: "active",
    rating: 4.9,
    commodities: ["Daging Sapi", "Tulang Sapi"],
    description: "Distributor khusus daging sapi impor dan lokal dengan fasilitas cold storage modern untuk menjaga mata rantai pendingin.",
    image: vendorImg,
    phone: "021-6667778",
  },
  {
    id: "v7",
    name: "Pusat Sayur Mayur Kramat Jati",
    type: "vendor",
    lat: -6.27,
    lng: 106.87,
    address: "Pasar Induk Kramat Jati, Jakarta Timur",
    status: "active",
    rating: 4.8,
    commodities: ["Sayuran Segar", "Cabai", "Bawang"],
    description: "Hub utama sayur mayur di Jakarta Timur, mengumpulkan hasil bumi terbaik dari berbagai wilayah di Indonesia.",
    image: vendorImg,
    phone: "021-8889990",
  },
  {
    id: "v8",
    name: "Agen Telur Berkah",
    type: "vendor",
    lat: -6.2,
    lng: 106.78,
    address: "Palmerah, Jakarta Barat",
    status: "active",
    rating: 4.6,
    commodities: ["Telur Ayam", "Telur Bebek"],
    description: "Pemasok telur terpercaya dengan pengawasan kualitas cangkang dan kebersihan produk yang ketat.",
    image: vendorImg,
    phone: "021-5556667",
  },
  // School Data
  {
    id: "s1",
    name: "SDN Menteng 01",
    type: "school",
    lat: -6.188,
    lng: 106.832,
    address: "Jl. Besuki No.4, Menteng, Jakarta Pusat",
    capacity: 450,
    status: "active",
    description: "Sekolah Dasar Negeri percontohan di Jakarta Pusat yang mengutamakan kesehatan dan gizi siswa melalui program makanan bergizi.",
    image: schoolImg,
    phone: "021-5550001",
  },
  {
    id: "s2",
    name: "SMPN 1 Jakarta",
    type: "school",
    lat: -6.175,
    lng: 106.82,
    address: "Jl. Cikini Raya No.87, Jakarta Pusat",
    capacity: 800,
    status: "active",
    description: "Salah satu SMP tertua di Jakarta yang memiliki standar kantin gizi seimbang untuk menunjang tumbuh kembang remaja.",
    image: schoolImg,
    phone: "021-5550002",
  },
  {
    id: "s3",
    name: "SDN Tebet Barat 03",
    type: "school",
    lat: -6.232,
    lng: 106.857,
    address: "Jl. Tebet Barat IX, Jakarta Selatan",
    capacity: 380,
    status: "active",
    description: "Sekolah dasar dengan program makan siang bergizi yang dipasok dari dapur SPPG terdekat di kawasan Tebet.",
    image: schoolImg,
    phone: "021-5550003",
  },
  {
    id: "s4",
    name: "SDN Senen 05",
    type: "school",
    lat: -6.184,
    lng: 106.842,
    address: "Jl. Kramat Bunder, Senen, Jakarta Pusat",
    capacity: 300,
    status: "active",
    description: "Sekolah dasar di pusat kota dengan akses cepat ke dapur pusat Senen untuk distribusi makanan harian.",
    image: schoolImg,
    phone: "021-5550004",
  },
  {
    id: "s5",
    name: "SMPN 19 Jakarta",
    type: "school",
    lat: -6.205,
    lng: 106.84,
    address: "Jl. Bumi, Manggarai, Jakarta Selatan",
    capacity: 620,
    status: "active",
    description: "Sekolah menengah dengan kebutuhan porsi besar yang menjadi prioritas dalam perencanaan distribusi.",
    image: schoolImg,
    phone: "021-5550005",
  },
  {
    id: "s6",
    name: "SDN Kebayoran 07",
    type: "school",
    lat: -6.243,
    lng: 106.799,
    address: "Jl. Gandaria, Kebayoran Baru, Jakarta Selatan",
    capacity: 420,
    status: "active",
    description: "Sekolah dasar di Jakarta Selatan yang dilayani oleh dapur Kebayoran dengan menu gizi seimbang.",
    image: schoolImg,
    phone: "021-5550006",
  },
  {
    id: "s7",
    name: "SDN Kramat Jati 02",
    type: "school",
    lat: -6.273,
    lng: 106.865,
    address: "Jl. Raya Bogor, Kramat Jati, Jakarta Timur",
    capacity: 350,
    status: "active",
    description: "Sekolah dasar di Jakarta Timur dekat pasar induk, dengan akses bahan baku sayur yang melimpah.",
    image: schoolImg,
    phone: "021-5550007",
  },
  // Kitchen Data
  {
    id: "k1",
    name: "Dapur Pusat Senen",
    type: "kitchen",
    lat: -6.192,
    lng: 106.845,
    address: "Pasar Senen Lantai 4, Jakarta Pusat",
    capacity: 2000,
    status: "active",
    rating: 4.7,
    description: "Pusat pengolahan makanan skala besar yang melayani distribusi ke 10 sekolah di wilayah Senen dan sekitarnya dengan standar higienis tinggi.",
    image: kitchenImg,
    phone: "021-7890001",
  },
  {
    id: "k2",
    name: "Dapur Sehat Tebet",
    type: "kitchen",
    lat: -6.225,
    lng: 106.855,
    address: "Jl. Tebet Barat Dalam, Jakarta Selatan",
    capacity: 1500,
    status: "active",
    rating: 4.5,
    description: "Dapur pengolah yang berfokus pada menu makanan bergizi seimbang dengan pengawasan ahli gizi tersertifikasi.",
    image: kitchenImg,
    phone: "021-7890002",
  },
  {
    id: "k3",
    name: "Dapur Pusat Kebayoran",
    type: "kitchen",
    lat: -6.241,
    lng: 106.802,
    address: "Jl. Sultan Iskandar Muda, Kebayoran Baru, Jakarta Selatan",
    capacity: 1800,
    status: "active",
    rating: 4.6,
    description: "Dapur SPPG modern yang melayani sekolah-sekolah di Jakarta Selatan bagian barat dengan kapasitas produksi besar.",
    image: kitchenImg,
    phone: "021-7890003",
  },
];
